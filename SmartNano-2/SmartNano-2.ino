...
//        receivepacket loop, so wrprot(ACK) was always 1. Added timeout support for  Serial.begin(230400);
...
// 1.17a Unknown
//
// V1.0 05/02/2026  - Renamed project to SmartNano
//
// V1.1 06/02/2026 -  Added support for the four LED version from SP2SD showing the boot device at next reset.
//                    Changed the selection procedure, you may press the eject button to rotate round the 4 images
//                    you only need to reset the board (and the apple if your powering it from the board).
// V1.2 06/02/2026 -  Removed 'status' LED replaced with the current partion LED doing the same thing as status
//                    Showing disk acivity etc.
// V1.3 11/02/2026 -  Added code to use the last drive  LED used in both Read and Write functions. Cleaned up
//                    code removed a lot of redundant comments.
//
...
int count;
unsigned int initPartition,statusledPin; // moving to use 4xled for status and use.
...
unsigned int partition_led_pins[4] = {14, 15, 16, 8}; // these where added by Kay Koba to indicate the next boot up image
...
//dcdcdcdcdc const uint8_t statusledPin = 18;
...
void setup() {
  ...
  mcuInit();
  Serial.begin(230400); // fast as possible avoid possible delays / error due to sending serial
  Serial.print(F("\r\nSmartNano v1.3\r\n"));
  
  pinMode((uint8_t)18,INPUT); //fidle to turn of old status led dcdcdcdcdcdc

  for(unsigned char i=0; i<4; i++){ //all led for partions are output
  ...
  statusledPin = initPartition; // Use this as the led number for traffic and status.

  digitalWrite(partition_led_pins[statusledPin], HIGH); // turn on the current boot partion LED dcdcdcdcdc
      ...
      case 0x0f:
        ...
        switch (packet_buffer[14]) {
          ...
          case 0x80:  //is a status cmd
            ...
            digitalWrite(partition_led_pins[statusledPin], LOW); //dcdcdcdc
            ...
            for (partition = 0; partition < NUM_PARTITIONS; partition++) { //Check if its one of ours
              if (devices[(partition + initPartition) % NUM_PARTITIONS].device_id == source
                ...
                digitalWrite(partition_led_pins[statusledPin], HIGH); //dcdcdcdcdc
              }
            }
            break;
          ...
          case 0xC0:  //Extended status cmd
           // digitalWrite(statusledPin, HIGH);
           digitalWrite(partition_led_pins[statusledPin], LOW); //dcdcdcdcdc
            source = packet_buffer[6];
            ...
            for (partition = 0; partition < NUM_PARTITIONS; partition++) { //Check if its one of ours
              if (devices[(partition + initPartition) % NUM_PARTITIONS].device_id == source) {  //yes it is, then reply
                ...
                //Serial.print(F("\r\nStatus CMD"));
                //digitalWrite(statusledPin, LOW);
                digitalWrite(partition_led_pins[statusledPin], HIGH); //dcdcdcdcdcdc
              }
            }
            //Serial.print(F("\r\nHere's our reply!"));
            //print_packet ((unsigned char*) packet_buffer, packet_length());
          ...
          case 0x81:  //is a readblock cmd
            ...
            for (partition = 0; partition < NUM_PARTITIONS; partition++) { //Check if its one of ours
              if (devices[(partition + initPartition) % NUM_PARTITIONS].device_id == source) {  //yes it is, then do the read
                ...
                //digitalWrite(statusledPin, HIGH);
                digitalWrite(partition_led_pins[statusledPin], LOW); //turn of previous LEDdcdcdcdcdcdc
                
                statusledPin = (partition + initPartition) % NUM_PARTITIONS; //Set new LED drive partition number.
                
                //Serial.print("\r\nLEDPIN ");
                //Serial.print(statusledPin,HEX);
...
////////<Wing>
                ...
                //digitalWrite(statusledPin, LOW);
                digitalWrite(partition_led_pins[statusledPin], HIGH); //dcdcdcdc
...
////////<Wing>
              ...
              //digitalWrite(statusledPin, LOW);
              digitalWrite(partition_led_pins[statusledPin], HIGH); //dcdcdcdcdcdcdc
...
void print_hd_info(void)
{
  ...
  if(!sdcard.begin(chipSelect, SPI_HALF_SPEED)){
    ...
    //digitalWrite(statusledPin, HIGH);
    digitalWrite(partition_led_pins[statusledPin], LOW);//dcdcdcdcdcdc
    delay(100);
    //digitalWrite(statusledPin, LOW);
    digitalWrite(partition_led_pins[statusledPin], HIGH); //dcdcdcdcdc
    delay(100);
...
int rotate_boot (void)
{
  ...
  while(digitalRead(ejectPin) == HIGH) delay(25); //wait for the button to be released and add in a debouce.
  //delay(50);// more debounce 
  //This code skips through the open partion files and stops at the next opened one
  digitalWrite(partition_led_pins[statusledPin], LOW); //Turn off old partion LED
  Serial.print(F("\r\nInit partition was: "));
  Serial.print(initPartition, DEC);
  ...
  }// Break comes here when a partion is found
  
  //check if no partions have been found, fail with an error and wait forever.
  if(i == NUM_PARTITIONS){
    Serial.print(F("\r\nNo online partitions found. Check config.txt or files called PARTx.PO and try again, where x is from 1 to "));
    Serial.print(NUM_PARTITIONS, DEC);
    initPartition = 0;
    // No partions found on the SD Card, this should not really happen.
    led_err(); // stop the program show errors on LED's
  } 

  //Found the next one Update the EEPROM for next reset
  eeprom_write_byte(0, initPartition);
  Serial.print(F("\r\nChanging boot partition to: "));
  Serial.print(initPartition, DEC);
  digitalWrite(partition_led_pins[initPartition], HIGH);
  statusledPin = initPartition;
}//end of function
...
void led_err(void)
{
  ...
  interrupts();
  Serial.print(F("\r\nError!"));
  //dcdcdcdcdc pinMode(statusledPin, OUTPUT);

  for (i = 0; i < 10; i++) {
    //digitalWrite(statusledPin, HIGH);
     for (l=0; l <4; l++) digitalWrite(partition_led_pins[l], HIGH);
    delay(500);
    //digitalWrite(statusledPin, LOW);
     for (l=0; l <4; l++) digitalWrite(partition_led_pins[l], LOW);
    delay(100);
  }
  while(1); // halt
}
