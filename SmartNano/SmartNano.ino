...
int rotate_boot (void)
{
  ...
  if(digitalRead(ejectPin) == HIGH) { // Button is pressed
   delay(300); // debounce
  ...
  if(i == NUM_PARTITIONS){
    ...
    while(1); // stop the program for ever.
  } 
...
void led_err(void)
{
  ...
  for (i = 0; i < 5; i++) {
    //digitalWrite(statusledPin, HIGH);
     digitalWrite(partition_led_pins[statusledPin], HIGH);
    delay(1500);
    //digitalWrite(statusledPin, LOW);
     digitalWrite(partition_led_pins[statusledPin], LOW);
    delay(100);
    //digitalWrite(statusledPin, HIGH);
     digitalWrite(partition_led_pins[statusledPin], HIGH);
    delay(1500);
